...
```bash
API_KEY=<ENTER_API_KEY_HERE> curl -X 'GET' \
...
OpenCode supports custom model providers through the `opencode.json` configuration file. To configure OpenCode to use Bedrock:

1. Create or edit the `opencode.json` file in your project root (or globally at `~/.config/opencode/opencode.json`)
...
3. Replace `your-api-key-from-setup` with the API key generated by `./setup.sh`
4. Add any additional Bedrock models you want to use in the `models` section
5. Reload OpenCode or restart the editor to apply the changes
6. Use the `/model` command in OpenCode to select which Bedrock model to use

![OpenCode Model Selection](opencode-model.png)

![OpenCode Bedrock Configuration](opencode-bedrock.png)
